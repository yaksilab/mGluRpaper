...
%% Other odor data (Suppl Fig1)

% to collect the BAodor data 
fishO = 3; %3;
hab_resp = collected_data_fish.resp_odorBA{fishO,1}; 
pos_cells = find(hab_resp== 1);
neg_cells = find(hab_resp== -1);
hab_pos = collected_data_fish.position_odorBA{fishO,1};
HB.DFFON = hab_pos(pos_cells,:); 
HB.DFFInh = hab_pos(neg_cells,:);


HB.DFFInh_aver = squeeze(mean(collected_data_fish.dff_odorBA{fishO,1}(1:100,:,neg_cells),2))'; 
HB.DFFON_aver = squeeze(mean(collected_data_fish.dff_odorBA{fishO,1}(1:100,:,pos_cells),2))';  

tit = 'Odor BA'

% to collect the odor data 
fishO = 7; %3 6
hab_resp = collected_data_fish.resp_odorFOOD{fishO,1}; 
pos_cells = find(hab_resp== 1);
neg_cells = find(hab_resp== -1);
hab_pos = collected_data_fish.position_odorFOOD{fishO,1};
HB.DFFON = hab_pos(pos_cells,:); 
HB.DFFInh = hab_pos(neg_cells,:);


HB.DFFInh_aver = squeeze(mean(collected_data_fish.dff_odorFOOD{fishO,1}(1:100,:,neg_cells),2))'; 
HB.DFFON_aver = squeeze(mean(collected_data_fish.dff_odorFOOD{fishO,1}(1:100,:,pos_cells),2))';  
...
subplot('Position',[0.01,0.86,0.1,0.04]); %plot(mean(HB.DFFInh_aver(:,:)), 'b'), 
H1=shadedErrorBar([1:size(HB.DFFInh_aver(:,:),2)],squeeze(mean(HB.DFFInh_aver(:,:))),squeeze(nanstd(HB.DFFInh_aver(:,:),0,1)/sqrt(size(HB.DFFInh_aver(:,:),1))), 'lineProps','r');
H1.mainLine.Color = 'b'; %'#A23333';
H1.patch.FaceColor = 'b'; %'#A23333'
axis tight
subplot('Position',[0.01,0.66,0.1,0.04]); %plot(mean(HB.DFFON_aver(:,:)), 'r'), axis tight
H1=shadedErrorBar([1:size(HB.DFFON_aver(:,:),2)],squeeze(mean(HB.DFFON_aver(:,:))),squeeze(nanstd(HB.DFFON_aver(:,:),0,1)/sqrt(size(HB.DFFON_aver(:,:),1))), 'lineProps','r');
H1.mainLine.Color = 'r'; %'#A23333';
H1.patch.FaceColor = 'r'; %'#A23333'
axis tight
...
% for the other odours
saveas(gcf, fullfile(save_path, 'BAOdour_Hb_heatmapshot.png'))
saveas(gcf, fullfile(save_path, 'BAOdour_Hb_heatmapshot.svg'))  

saveas(gcf, fullfile(save_path, 'BAOdour_Hb_3D.png'))
saveas(gcf, fullfile(save_path, 'BAOdour_Hb_3D.svg'))

saveas(gcf, fullfile(save_path, 'foododour_Hb_heatmapshot.png'))
saveas(gcf, fullfile(save_path, 'foododour_Hb_heatmapshot.svg'))  

saveas(gcf, fullfile(save_path, 'foodOdour_Hb_3D.png'))
saveas(gcf, fullfile(save_path, 'foodOdour_Hb_3D.svg'))
...
%% For different odors (Suppl Fig1)

[neg_perc_resp_o, neg_perc_resp_oBA, neg_perc_resp_oFOOD] = plot_perc_resp_cells_AO(collected_data_fish.resp_odor', collected_data_fish.resp_odorBA', collected_data_fish.resp_odorFOOD', 1, cmap1, cmap1,cmap1, -1, {'Stuff'});
close;
[pos_perc_resp_o, pos_perc_resp_oBA, pos_perc_resp_oFOOD] = plot_perc_resp_cells_AO(collected_data_fish.resp_odor', collected_data_fish.resp_odorBA', collected_data_fish.resp_odorFOOD', 1, cmap1, cmap1, cmap1, 1, {'Stuff'});
close;
...
% Odor BA
err_odBA_pos = std(pos_perc_resp_oBA(:,:), 0 ,1)/sqrt(size(pos_perc_resp_oBA,1));
err_odBA_neg = std(neg_perc_resp_oBA(:,:), 0 ,1)/sqrt(size(neg_perc_resp_oBA,1));

figure('units','centimeters','Position',[2 2 8 10])
hold on 
AO_make_bar_scatter_line_plot(pos_perc_resp_oBA', neg_perc_resp_oBA', err_odBA_pos, err_odBA_neg, cmap1, 'OdorBA', 'Exc', 'Inh')
ylim([0 50])
saveas(gcf, fullfile(save_path, 'BAOd_perc.png'))
saveas(gcf, fullfile(save_path, 'BAOd_perc.svg')) 

figure('units','centimeters','Position',[2 2 10 5]) 
donout = donut([100-(mean(pos_perc_resp_oBA) + mean(neg_perc_resp_oBA)), mean(pos_perc_resp_oBA),mean(neg_perc_resp_oBA)],{'Non','Pos', 'Neg'}, cmap1)
legend('Location', 'Eastoutside')
title('Odor BA')
saveas(gcf, fullfile(save_path, 'BAOd_perc_pie.png'))
saveas(gcf, fullfile(save_path, 'BAOd_perc_pie.svg'))

% stats 
[p, h] = quick_statistic_signrank(pos_perc_resp_oBA, neg_perc_resp_oBA)



% Odor FOOD
err_odFOOD_pos = std(pos_perc_resp_oFOOD(:,:), 0 ,1)/sqrt(size(pos_perc_resp_oFOOD,1));
err_odFOOD_neg = std(neg_perc_resp_oFOOD(:,:), 0 ,1)/sqrt(size(neg_perc_resp_oFOOD,1));

figure('units','centimeters','Position',[2 2 8 10])
hold on 
AO_make_bar_scatter_line_plot(pos_perc_resp_oFOOD', neg_perc_resp_oFOOD', err_odFOOD_pos, err_odFOOD_neg, cmap1, 'Odorfood', 'Exc', 'Inh')
ylim([0 50])
saveas(gcf, fullfile(save_path, 'foodOd_perc.png'))
saveas(gcf, fullfile(save_path, 'foodOd_perc.svg')) 

figure('units','centimeters','Position',[2 2 10 5]) 
donout = donut([100-(mean(pos_perc_resp_oFOOD) + mean(neg_perc_resp_oFOOD)), mean(pos_perc_resp_oFOOD),mean(neg_perc_resp_oFOOD)],{'Non','Pos', 'Neg'}, cmap1)
legend('Location', 'Eastoutside')
title('Odor food')
saveas(gcf, fullfile(save_path, 'foodOd_perc_pie.png'))
saveas(gcf, fullfile(save_path, 'foodOd_perc_pie.svg'))

% stats 
[p, h] = quick_statistic_signrank(pos_perc_resp_oFOOD, neg_perc_resp_oFOOD)