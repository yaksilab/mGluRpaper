function [resp_corr_val, similarity_val] = response_vector_correl(all_dfftrialw_wt_FL, brain_regions_wt, position_wt, stim_period, con_trials, resp_list_wt_dff_std, dorsomed, thresh_dormedneurons_wt, brainnumber)


resp_corr_val = nan(size(all_dfftrialw_wt_FL,2),3); 
similarity_val = nan(size(all_dfftrialw_wt_FL,2),3); 
...
for fish = 1:size(all_dfftrialw_wt_FL,2)
    ...
    similarities_LT = cosineSimilarity(resp_vector(:,1)',resp_vector(:,2)')
    similarities_LLT = cosineSimilarity(resp_vector(:,1)',resp_vector(:,3)')
    similaritiesTLT = cosineSimilarity(resp_vector(:,2)',resp_vector(:,3)')
    ...
    similarity_val(fish,1) = similarities_LT; 
    similarity_val(fish,2) = similarities_LLT; 
    similarity_val(fish,3) = similaritiesTLT; 
