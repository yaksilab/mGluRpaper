function plotstackedthreecon(perc_resp_wt, perc_resp_het, perc_resp_hom, map_wt, map_het, map_hom, con_names, group_names, tit, width, height)
...
hold on
b = bar([1 2 3],[nanmean(perc_resp_wt(:,1)) nanmean(perc_resp_wt(:,2)); nanmean(perc_resp_het(:,1)) nanmean(perc_resp_het(:,2)); nanmean(perc_resp_hom(:,1)) nanmean(perc_resp_hom(:,2))], 'stacked')
...
er.Color = [0 0 0];                            
er.LineStyle = 'none'; 
er = errorbar([3, 3],[nanmean(perc_resp_hom(:,1)), nanmean(perc_resp_hom(:,1))+nanmean(perc_resp_hom(:,2))],-err_hom(1:2),err_hom(1:2));    
er.Color = [0 0 0];                            
er.LineStyle = 'none'; 


xticks(gca, [1 2 3])
xticklabels({group_names{[1 2 3]}})
legend([b], {'Uni', 'Mulit'})
